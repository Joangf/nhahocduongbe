...
public class DashboardController {
    ...
    private List<Map<String, Object>> calculateTopSchools(List<Map<String, Object>> schoolClassStats) {
        // Group by School Name/ID to get aggregates per school
        ...
        // Sort by cariesRate descending, limit to top 5
    ...
    private Map<String, Integer> calculatePathologyHeatmap(List<Exam> exams) {
        ...
        // Initialize teeth map (common teeth positions 11-18, 21-28, 31-38, 41-48)
        int[] regions = {1, 2, 3, 4};
        for (int r : regions) {
            for (int i = 1; i <= 8; i++) {
                heatmap.put("" + r + i, 0);
            ...
            if (tr != null && tr.getRecord() != null) {
                for (Map.Entry<Tooth, ToothCondition> entry : tr.getRecord().entrySet()) {
                    ...
                    if (cond != null && cond.getProblem() == ToothProblem.CARIES) {
                        ...
                        // toothName is like "T_11", "T_12", etc., or "11", "12". Let's extract numeric part

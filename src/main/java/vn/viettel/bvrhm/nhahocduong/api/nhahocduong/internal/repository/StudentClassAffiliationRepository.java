...
public interface StudentClassAffiliationRepository extends JpaRepository<StudentClassAffiliation, Long> {
  ...
  @Query(value = """
      ...
      ORDER BY o.name, c.grade
      """, nativeQuery = true)
  List<Object[]> countStudentsBySchoolAndGradeFiltered(@Param("yearId") Long academicYearId, @Param("orgId") Long organizationId);
  ...
  @Query(value = """
      ...
      JOIN academic_year ay ON ay.id = a.academic_year_id
      JOIN class c ON c.id = a.class_id
      WHERE a.status = 'STUDYING' AND c.school_id = :orgId
      GROUP BY ay.name, ay.start_date
      ORDER BY ay.start_date
      """, nativeQuery = true)
  List<Object[]> countStudentsByYearFiltered(@Param("orgId") Long organizationId);
